...
class Server extends Base
{
    public function execute(
        ...
        if ('ssh' === $type) {
            ...
            if (null !== $sshUser) {
                $this->config->set($name, 'user', $sshUser);
            }

            if (null !== $sshAuth) {
                $this->config->set($name, 'auth', $sshAuth);
            }

            if ('password' === $sshAuth) {
                if (null !== $sshPassword) {
                    $this->config->set($name, 'password', $sshPassword);
                }
            } elseif ('key' === $sshAuth) {
                if (null !== $sshPrivateKey) {
                    $this->config->set($name, 'privateKey', $sshPrivateKey);
                }
            } elseif ('file' === $sshAuth) {
                if (null !== $sshPrivateKeyFile) {
                    $this->config->set($name, 'privateKeyFile', $sshPrivateKeyFile);
                }
            }
        }

        $this->config->set($name, 'shell', $shell);
    }
}
